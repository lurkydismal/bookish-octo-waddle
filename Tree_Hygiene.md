...
bookish-octo-waddle other than the actual writing of the code. For guidance on
designing APIs, documenting, and formatting your code, see the
[Style guide for bookish-octo-waddle repo](Style_Guide.md) document.

## Overview

The general process for submitting code to a bookish-octo-waddle repository is as follows:
...
1. Create a branch off of `main` (or if the repository has not yet switched to
    ...
    You must follow the guidelines described in the [Style guide for bookish-octo-waddle repo](Style_Guide.md).
...
1. Submit this branch as a PR to the relevant bookish-octo-waddle repository.
...
rolling a dependency. Getting a review means that a regular bookish-octo-waddle
...
[bookish-octo-waddle style guide](Style_Guide.md).
...
If you are not a regular bookish-octo-waddle contributor (someone with commit access),
...
Sometimes the contributor is unable to finish the work of landing the patch. In that case, if the PR has promise, we may close it but mention it on the relevant issue so that other interested parties can pick it up. Such issues are given the label [`has partial patch`](https://github.com/lurkydismal/bookish-octo-waddle/labels/has%20partial%20patch).
...
\deprecated Call prepareFrame followed by owner.requestVisualUpdate() instead.
  This will enable an improvement to performance in a future version of bookish-octo-waddle.
